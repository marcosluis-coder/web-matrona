...
def register():
    if request.method == "POST":
        ...
        user = Usuari(nom=nom, email=email, contrasenya=hashed_password, rol=rol)
        db.session.add(user)
        db.session.commit()
...
def nova_cita():
    ...
    if request.method == "POST":
        ...
        if data_cita < today_date:
            ...
            return redirect(url_for("main.nova_cita", data=data_str))
        ...
        # ❌ HORA PASSADA (si és avui)
        if data_cita == today_date and hora_cita <= now_time:
            flash("No pots reservar una hora passada")
            return redirect(url_for("main.nova_cita", data=data_str))

        # ❌ CITA DUPLICADA
        cita_existente = Cita.query.filter_by(data=data_str, hora=hora_str).first()
        if cita_existente:
            flash("Aquesta hora ja està reservada")
            return redirect(url_for("main.nova_cita", data=data_str))

        # ✔ CREAR CITA
        cita = Cita(data=data_str, hora=hora_str, usuari_id=current_user.id)
        db.session.add(cita)
        db.session.commit()
    ...
    if data_seleccionada:
        data_cita = datetime.datetime.strptime(data_seleccionada, "%Y-%m-%d").date()

        # eliminar hores ocupades
        cites = Cita.query.filter_by(data=data_seleccionada).all()
        hores_ocupades = [c.hora for c in cites]

        hores_disponibles = [h for h in hores_base if h not in hores_ocupades]

        # eliminar hores passades si és avui
        if data_cita == today_date:
            hores_disponibles = [
                h for h in hores_disponibles
                if datetime.datetime.strptime(h, "%H:%M").time() > now_time
            ]
...
@login_required
def mis_citas():
    citas = Cita.query.filter_by(usuari_id=current_user.id).all()
...
def totes_cites():
    ...
    citas = Cita.query.order_by(Cita.data, Cita.hora).all()
    return render_template("totes_cites.html", citas=citas)
...
def eliminar_cita(id):
    ...
    if current_user.rol == "pacient" and cita.usuari_id != current_user.id:
        return "Accés denegat"
...
def dashboard():
    ...
    cites_avui = Cita.query.filter_by(data=avui).all()
    pendents = Cita.query.filter_by(estat="pendent").count()
    confirmades = Cita.query.filter_by(estat="confirmada").count()
    cancelades = Cita.query.filter_by(estat="cancelada").count()
    ...
    if request.method == "POST":
        ...
        if data_str:
            ...
            if tipus == "embaras":
                ...
                propera_regla = data + datetime.timedelta(days=28)
